...
<td class="forminp">
    <div id="etwc_req_input" class="etwc-datainputs">
        <?php if ( ! empty( $field_Val ) && is_array( $field_Val ) ) : ?>
            <?php foreach ( $field_Val as $index => $data ) : ?>
                <div class="ekwc-input-group" style="margin-bottom: 10px;">
                    ...
                    <?php if( $index > 0 ) : ?>
                        <button type="button" class="etwc-remove-field button"> <?php esc_html_e( 'Remove', 'essential-kit-for-woocommerce' ); ?> </button>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
        <?php else : ?>
    ...
    <button type="button" id="etwc_add_field" class="button" style="margin-top: 10px;"><?php esc_html_e( 'Add More', 'essential-kit-for-woocommerce' ); ?></button>
</td>
